...
const char* ssid     = "esp_iot_test"; //enter your ssid/ wi-fi(case sensitiv) router name - 2.4 Ghz only
const char* password = "esptesting";     // enter ssid password (case sensitiv)
char host[] = "tushariot.herokuapp.com"; //enter your Heroku domain name like "espiot.herokuapp.com" 
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
void webSocketEvent(WStype_t type, uint8_t * payload, size_t length) { //uint8_t *
    ...
    switch(type) {
        ...
        case WStype_BIN:
         ...
         default:
         Serial.print("NO use case Found");
         break;
    }
...
void processWebScoketRequest(String data){
            ...
            if(query == "cmd"){ //if query check state
              Serial.println("Recieved command!");
                    if(state=="on"){
                      Serial.println("Receieved command 123145");
                      digitalWrite(relayPin, HIGH);
                      message = "{\"state\":\"ON\"}";
                      currState = "ON";
                    }else{
                      Serial.println("Receieved command 8798799");
